...
ns = []
for n in range(len(sample_text)):
    if sample_text[n] != "":
        ns.append(sample_text[n])
sample_text = ns
...
text = text + random.choices(m2lf1[0], weights=m2lf1[1])

m2lf11 = makeTwoLists(f_11[text[0]])
text = text + random.choices(m2lf11[0], weights=m2lf11[1])

print(text)
prev2 = (text[0] , text[1])
for i in range(textLength):
    m2lf21 = makeTwoLists(f_21[prev2])
    addon = random.choices(m2lf21[0], weights=m2lf21[1])
    text = text + addon
    prev2 = (prev2[1], addon[0])
